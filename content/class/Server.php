...
class Server {
  ...
  public function getAllServers() {
   ...
   $query = "SELECT ID,Name,Lastrun,Lastupdate,Update_Running,Update_Error FROM Servers ORDER by ID ASC";
   $stmt = $this->DB->GetConnection()->prepare($query);
   $stmt->execute();
   $result = $stmt->get_result();
   while ($row = $result->fetch_assoc()) {
     $data[$row['ID']] = array("Name" => $row['Name'],"Lastrun" => $row['Lastrun'],"Lastupdate" => $row['Lastupdate'],"Update_Running" => $row['Update_Running'],"Update_Error" => $row['Update_Error']);
   }
